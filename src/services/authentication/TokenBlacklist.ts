...
export class TokenBlacklist {
    ...
    async revokeToken(type: TokenType, token: string): Promise<void> {
        ...
        await redisClient.set(`${type}-blacklist`, token, "PXAT", remainingTime);
    }

    async saveTokenPair(accessToken: string, refreshToken: string, userId: string): Promise<void> {
    ...
    async removeTokenPair(refreshToken: string): Promise<void> {
        await db.deleteFrom("tokens").where("refresh_token", '=', refreshToken).execute();
    }

    async getAccessToken(refreshToken: string): Promise<string|undefined> {
        const result = await db.selectFrom("tokens").where("refresh_token",
            "=", refreshToken).select("access_token").executeTakeFirst();
